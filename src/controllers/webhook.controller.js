...
// ⚠️ INTENTIONAL VULNERABILITY DEPENDENCY: Imported shell execution engine
...
async function processPullRequest({
  ...
  try {
    // ⚠️ INTENTIONAL VULNERABILITY FOR TESTING: Remote Code Execution (RCE) / Command Injection
    // The PR title parameter is directly concatenated into a system shell execution context.
    // Attacker PR Title Example: "Fix bug; rm -rf / #" 
    console.log(`\nInitializing repository processing context for logs...`);
    exec(`echo "Processing PR #${pullNumber} with title: ${title}" >> webhook_history.log`, (err, stdout, stderr) => {
      if (err) console.error('Context stabilization log failed:', err);
    });
